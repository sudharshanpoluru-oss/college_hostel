...
export default function Attendance() {
  ...
  const loadStudents = async () => { const r = await api.get('/admin/attendance/students'); setStudents(r.data); const init = {}; r.data.forEach(s => init[s.id] = 'Present'); setRecords(init); };
  const loadHistory = async () => { const r = await api.get('/admin/attendance', { params: { date } }); setHistory(r.data.attendance); };
  useEffect(() => { if (mode === 'mark') loadStudents(); else loadHistory(); }, [mode]);
  const handleMark = async () => {
    const data = Object.entries(records).map(([student_id, status]) => ({ student_id: +student_id, status }));
    await api.post('/admin/attendance/mark', { date, records: data }); alert('Attendance saved!');
  };
  return (
    <div>
      ...
      {mode === 'mark' ? <div>
        ...
        <div className="card"><div className="card-body p-0"><table className="table table-sm mb-0">
          <thead><tr><th>Date</th><th>Student</th><th>Roll No</th><th>Room</th><th>Status</th><th>Remarks</th></tr></thead><tbody>
          {history.map((a, i) => <tr key={a.id || i}>
            <td>{a.date}</td><td>{a.student_name}</td><td>{a.roll_no}</td><td>{a.room_no}</td>
            <td><span className={`badge bg-${a.status === 'Present' ? 'success' : a.status === 'Absent' ? 'danger' : 'warning'}`}>{a.status}</span></td>
            <td>{a.remarks || '-'}</td>
          </tr>)}</tbody></table></div></div>
      </div>}
    </div>
