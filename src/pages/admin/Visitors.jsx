...
export default function Visitors() {
  ...
  const load = () => { api.get('/admin/visitors', { params: { page, search } }).then(r => setData(r.data)); };
  useEffect(() => { load(); }, [page, search]);
  const handleSubmit = async (e) => { e.preventDefault(); await api.post('/admin/visitors', form); setShowAdd(false); load(); };
  const handleCheckout = async (id) => { await api.put(`/admin/visitors/checkout/${id}`); load(); };
  return (
    <div>
